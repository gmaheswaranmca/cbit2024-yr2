# pip install pymysql # run from terminal
import pymysql                              # ch1

def connect():
    con = pymysql.connect(host='ocdb.app',port=5051,user='user_42kvxh2ws',
        password='p42kvxh2ws',database='db_42kvxh2ws')    # ch2/....
    return con 
def noteTablesCreate():
    sql = """CREATE TABLE IF NOT EXISTS notes(
        id integer primary key AUTO_INCREMENT /*ch3*/,
        title varchar(255) not null,
        notes varchar(2000) not null
    )"""
    con = connect()
    cur = con.cursor()          #ch 4 + 
    cur.execute(sql)            #ch 4 !
    con.close()
